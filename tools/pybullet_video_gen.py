...
# p_x = np.hstack((np.linspace(4, 4.5, int(num_frames/2)), np.linspace(4.5, 5, int(num_frames/2))))
# p_y = np.hstack((np.linspace(-2, -2, int(num_frames/2)), np.linspace(-2, -2, int(num_frames/2))))
...
# p_yaw = np.hstack((np.linspace(-np.pi/2, -np.pi/2, int(num_frames/2)), np.linspace(-np.pi/2, -np.pi/2, int(num_frames/2))))
#
...
def main():
    ...
    parser.add_argument("--fmap", help="specify the .fmap file with apriltag locations and orientations", default="../fmap/singletag.fmap")
